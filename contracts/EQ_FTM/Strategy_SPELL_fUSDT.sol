...
contract Strategy_SPELL_fUSDT {
    ...
    function _getPriceWFTMToSPELL() internal view returns (uint256 result) {
        ...
        uint256 amountFUSDT = _getAmountsOut(100 * 1e18, path);
        if (amountFUSDT > 0) {
            path[0] = token0;
            path[1] = token1;
            result = 1e36 / (_getAmountsOut(amountFUSDT, path) / 100);
        }
    }
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if(_totalLP > 0) {
            ...
            // calculate SPELL as fUSDC
            if(t1V != 0) {
                path[0] = token1;
                path[1] = token0;
                amountFUSDC = _getAmountsOut(t1V, path);
            }
            // calculate fUSDC to wFTM 
            if(t0V + amountFUSDC != 0) {
                path[0] = token0;
                path[1] = WETH;
                TC = _getAmountsOut(t0V + amountFUSDC, path);
            }
        }

        // calculates total Capital from tokens that exist on the contract
        
        // ----> calculates SPELL as fUSDC <----
        uint256 _balance = getBalanceOfToken(token1);
        amountFUSDC = 0;
        if(_balance != 0) {
            path[0] = token1;
            path[1] = token0;
            amountFUSDC = _getAmountsOut(_balance, path);
        }

        // ----> calculates fUSDC to wFTM <-----
        _balance = getBalanceOfToken(token0);
        if(_balance + amountFUSDC != 0) {
            path[0] = token0;
            path[1] = WETH;
            TC += _getAmountsOut(_balance + amountFUSDC, path);
        }
