...
contract StrategyDINO is Ownable {
    ...
    address public constant farm = 0x1948abC5400Aa1d72223882958Da3bec643fb4E5;
    ...
    function _swapNativeTokenToTokens() internal {
        ...
        uint256 _balance = getAvalaibleWETH() / 2;
    ...
    function requestWithdraw(address _reciever, uint256 _percent) public onlyVault {
    ...
    function migrate(uint256 _percent) public onlyVault {
        ...
        IERC20(WETH).transfer(vault, getAvalaibleWETH());
        updateTotalCapital();
    }
    ...
    function depositAsMigrate() public onlyVault {
        _approveToken(token0);
        _approveToken(token1);
        IERC20(token0).safeApprove(lpToken, 0);
        IERC20(token0).safeApprove(lpToken, type(uint256).max);
        IERC20(token1).safeApprove(lpToken, 0);
